...
===
Add attributes like `data-rapid-visible` and `data-rapid-display` assigned to a boolean value. Whenever that boolean value is changed, it will cause the element to be visible/hidden or display/none. The display value is saved (eg. inline-block, block, flex).
